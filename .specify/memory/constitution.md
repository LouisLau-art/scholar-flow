...
- **异常监控（Feature 027 - Sentry）**：
  ...
  - **Sourcemaps**：前端必须始终启用 `withSentryConfig` 以确保 config 注入与事件上报正常；线上构建如未设置 `SENTRY_AUTH_TOKEN` / `SENTRY_ORG` / `SENTRY_PROJECT`，则自动禁用 sourcemaps 上传（仍可用 DSN 上报事件）。
...
- **部署（Vercel + Render / Zeabur）**：
  ...
  - Zeabur 可直接使用仓库根目录 `Dockerfile`（`python:3.12-slim`），也支持命令模式（Root Directory `backend` + Start Command `uvicorn main:app --host 0.0.0.0 --port $PORT`）。
...
**Version**: 1.1.3 | **Ratified**: 2026-02-02 | **Last Amended**: 2026-02-05
...
- Portal：修复 `/api/v1/portal/articles/latest` 在 HF Space 上因 Supabase SDK 参数差异与云端 schema 漂移（缺 `authors`/`published_at`）导致的 500；作者展示字段由后端组装（profile 优先、Auth email 脱敏兜底），不返回 `Unknown` 且不泄露明文邮箱。
