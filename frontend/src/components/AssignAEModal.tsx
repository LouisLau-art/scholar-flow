  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => (!open ? onClose() : undefined)}>
      <DialogContent className="max-w-md">
        ...
        <div className="mb-4">
          <label htmlFor="assign-ae-search" className="block text-sm font-medium text-foreground mb-1">Search AE</label>
          ...
          <label className="block text-sm font-medium text-foreground mb-1">Select AE</label>
          <Select
          ...
          {!isLoadingAEs && filteredAes.length === 0 ? (
            <div className="mt-2 text-xs text-muted-foreground">没有匹配的 AE，请调整搜索关键词。</div>
          ) : null}
          {isLoadingAEs ? <div className="mt-2 text-xs text-muted-foreground">Loading assistant editors…</div> : null}
          {error ? <div className="mt-2 text-xs text-destructive">{error}</div> : null}
          <div className="mt-3 rounded-md border border-border bg-muted/50 px-3 py-2 text-xs text-foreground">
            分配后将自动进入 <code>under_review</code>。Owner 可选：不选则默认绑定为当前 ME（仅开发/UAT 提速）。
          </div>
        </div>

        <div className="mb-4">
          <label htmlFor="assign-owner-search" className="block text-sm font-medium text-foreground mb-1">Select Owner (Optional)</label>
          <div className="relative mb-3">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
            <Input
