...
interface ManuscriptPdfPreviewProps {
  pdfUrl: string | null
  isLoading: boolean
}

export function ManuscriptPdfPreview({ pdfUrl, isLoading }: ManuscriptPdfPreviewProps) {
  if (isLoading) {
    return (
  ...
  return (
    <Card className="overflow-hidden border-border shadow-sm">
      ...
      <CardContent className="p-0">
        <iframe title="Manuscript PDF" src={pdfUrl} className="h-[72vh] min-h-[520px] w-full" />
      </CardContent>
    </Card>
  )
}
