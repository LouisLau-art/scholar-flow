    ...
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="text-xs text-slate-500">{loading ? 'Loading…' : `${rows.length} reviewers`}</div>
        </div>
            ...
            {rows.length === 0 && !loading ? (
              ...
              rows.map((r) => (
                <tr key={r.id} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-700">{r.affiliation || '-'}</td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {(r.research_interests || []).length > 6 && (
                        <span className="text-xs text-slate-500">+{(r.research_interests || []).length - 6}</span>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3">
        ...
        <DialogContent className="max-w-md">
          ...
          {pendingRemove && (
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm">
              ...
              <div className="text-xs text-slate-600">{pendingRemove.email}</div>
            </div>
          )}
          <DialogFooter>
