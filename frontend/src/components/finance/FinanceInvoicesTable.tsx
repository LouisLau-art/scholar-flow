...
export function FinanceInvoicesTable(props: Props) {
  if (props.loading) {
    return <div className="p-8 text-sm text-slate-500">Loading invoices...</div>
  }

  if (!props.rows.length) {
    return <div className="p-8 text-sm text-slate-500">{props.emptyText || '当前无匹配账单'}</div>
  }
      ...
      <table className="w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-slate-200 bg-white">
          {props.rows.map((row) => {
            ...
            return (
              <tr key={row.invoice_id} className="hover:bg-slate-50 transition-colors">
                <td className="px-4 py-3 align-top">
                  <div className="font-medium text-slate-900">{row.invoice_number || row.invoice_id}</div>
                  <div className="text-xs text-slate-500">{row.invoice_id}</div>
                </td>
                <td className="px-4 py-3 align-top">
                  <div className="font-medium text-slate-900">{row.manuscript_title}</div>
                  <div className="text-xs text-slate-500">{row.authors || 'Author'}</div>
                </td>
                <td className="px-4 py-3 text-slate-700">{formatAmount(Number(row.amount || 0), row.currency || 'USD')}</td>
