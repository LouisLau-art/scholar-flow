...
test.describe('Reviewer invite accept flow (mocked)', () => {
  test('due date field uses native date input and stays editable before acceptance', async ({ page }) => {
    const assignmentId = '00000000-0000-0000-0000-000000000211'
    ...
    await expect(dueDateInput).toHaveValue('2026-02-07')
    await dueDateInput.fill('2026-02-08')
    await expect(dueDateInput).toHaveValue('2026-02-08')
  })
  ...
  test('accepts invitation and enters workspace', async ({ page }) => {
    ...
    await expect(page.getByLabel('Due date')).toHaveValue('2026-02-07')
    await Promise.all([
