...
test.describe('Reviewer Workspace layout (mocked)', () => {
  test('loads immersive layout with PDF + action panel', async ({ page }) => {
    ...
    const reviewCommentBox = await page.getByRole('heading', { name: 'Review Comment' }).boundingBox()
    const timelineBox = await page.getByRole('heading', { name: 'Communication Timeline' }).boundingBox()
    expect(reviewCommentBox).not.toBeNull()
    expect(timelineBox).not.toBeNull()
    expect(reviewCommentBox!.x).toBeLessThan(timelineBox!.x)
  })
})
