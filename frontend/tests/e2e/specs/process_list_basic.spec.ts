...
test.describe('Process list filtering (mocked backend)', () => {
  test('URL query params filter the list', async ({ page }) => {
    ...
    await page.goto('/editor/process?status=under_review')
    const table = page.getByTestId('editor-process-table')
    await expect(table).toBeVisible()
    await expect(table.getByText('11111111-1111-1111-1111-111111111111')).toBeVisible()
    await expect(table.getByText('Under Review')).toBeVisible()
    await expect(table.getByText('22222222-2222-2222-2222-222222222222')).not.toBeVisible()
