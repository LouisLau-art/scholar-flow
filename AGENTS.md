...
## Active Technologies
- Python 3.14+（本地），Python 3.12（HF Docker），TypeScript 5.x + FastAPI, Pydantic v2, Supabase PostgreSQL/Storage, Next.js 14, pytest, Playwright (043-production-cloud-rollout)
- Supabase PostgreSQL（新增 release validation 记录表），Supabase Storage（复用现有 bucket 验证） (043-production-cloud-rollout)
