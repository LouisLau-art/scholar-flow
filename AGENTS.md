...
- **Tailwind 设计系统化基线（2026-02-25）**：前端基线文档统一维护在 `docs/TAILWIND_V4_MIGRATION_BASELINE.md`；审计命令为 `cd frontend && bun run audit:tailwind-readiness`。当前基线：`w-[96vw]=0`、`hex=5`、`inline style=4`、`hard palette=421`（已完成第 5/6/7/8/9/10 批 token 化）。
...
## 近期关键修复快照（2026-02-25）
- **前端样式 token 化推进（2026-02-25）**：完成第 5/6/7/8/9/10 批高频页面改造（workspace/production/admin/reviewer/auth/decision 链路），统一替换 `slate|blue` 硬编码为语义 token；`hard palette` 从 `973` 降至 `421`，并保持 `w-[96vw]=0`。
