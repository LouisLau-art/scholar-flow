...
- **AI/ML**:
  ...
  - **DOCX 文本提取（本地）**：基于 `word/document.xml` 的轻量解析（见 `backend/app/core/docx_processor.py`）
  - **元数据抽取（Gemini 优先，DOCX-first）**：当前执行口径为 **DOCX-first, PDF-fallback**。上传稿件后优先调用 Gemini Developer API 结构化输出抽取 `title / abstract / authors`（`backend/app/core/gemini_metadata.py`，通过 `GEMINI_API_KEY` + `GEMINI_METADATA_MODEL` 启用，默认模型 `gemini-3.1-flash-lite-preview`）；若 Gemini 不可用或缺字段，则回退到本地规则解析（`backend/app/core/ai_engine.py`）。若 Word 稿成功解析出标题/摘要，则后续 PDF 上传仍作为必需文件但不再参与元数据竞争。
...
### 测试执行分层（提速约定）
- **高风险改动必须保留 TDD**：状态机、权限/RBAC、认证、邮件发送、数据库迁移、核心业务链路 bugfix 必须遵循“先失败测试，再最小实现，再回归验证”。
- **普通改动允许最小本地验证后直接 push**：页面流程、表单行为、列表展示、非核心 API 口径调整，至少运行相关定向测试 + `lint`/`tsc`/`ruff` 中的必要子集，再 push。
- **低风险改动可直接 push，由 CI 兜底**：纯文案、样式微调、文档、注释、非关键展示调整。
- **禁止把 CI 当本地开发环境替代品**：不能对高风险改动采取“先随手改、直接 push、等 CI 报错再理解业务”的工作方式。
- **目标不是测试最少，而是验证最小**：每轮改动只跑与改动范围直接相关的最小验证集合，避免整仓全量回归拖慢节奏。
...
- **Feature 048（Email Template Management）**：新增 `supabase/migrations/20260306110000_create_email_templates.sql`（`public.email_templates` + 默认模板种子）；后台新增 Admin 模板管理接口 `GET/POST/PUT/DELETE /api/v1/admin/email-templates` 与页面 `/admin/email-templates`；审稿邮件发送改为 `template_key` 动态查库渲染（支持 `{{ journal_title }}` 等变量），并按模板 `event_type` 回填 `review_assignments.invited_at/last_reminded_at`。
...
## Recent Changes
- 001-ui-guideline-remediation: Added TypeScript 5.x（strict）, Next.js 14.2（App Router）, React 18 + shadcn/ui（Radix primitives）, Tailwind CSS v4, next-themes, lucide-react
...
## Active Technologies
- TypeScript 5.x（strict）, Next.js 14.2（App Router）, React 18 + shadcn/ui（Radix primitives）, Tailwind CSS v4, next-themes, lucide-react (001-ui-guideline-remediation)
- N/A（本特性不新增存储结构） (001-ui-guideline-remediation)
