...
async def publish_manuscript(
    manuscript_id: UUID, is_finance_confirmed: bool, is_eic_approved: bool
):
    """
    稿件发布最终门控逻辑
    ...
    if not is_finance_confirmed:
        raise HTTPException(
            status_code=403, detail="发布拦截: 必须由财务人员手动确认到账后方可发布"
        )
    ...
    # Feature 015: Trigger DOI Registration
    try:
        ...
        if config:
            doi_service = DOIService(config)
            await doi_service.create_registration(manuscript_id)
            print(f"DOI registration triggered for {manuscript_id}")
    except Exception as e:
        print(f"Failed to trigger DOI registration: {e}")

    return {"id": manuscript_id, "status": "published", "published_at": datetime.now()}
