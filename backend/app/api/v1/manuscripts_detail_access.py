...
async def get_manuscript_versions_impl(
    manuscript_id: UUID,
    current_user: dict,
):
    """
...
async def get_manuscript_detail_impl(
    manuscript_id: UUID,
    current_user: dict,
    profile: dict,
):
    ...
    current_status = utils._m()._normalize_private_progress_status(ms.get("status"))
    if current_status:
    ...
    try:
        ...
        for row in log_rows:
            from_status = utils._m()._normalize_private_progress_status(row.get("from_status"))
            to_status = utils._m()._normalize_private_progress_status(row.get("to_status"))
            if from_status:
        ...
        if not utils._m()._is_missing_table_error(str(e), "status_transition_logs"):
            print(f"[ById] transition logs fallback failed (ignored): {e}")

    enriched = dict(ms)
    enriched["workflow_visited_statuses"] = utils._m()._order_private_progress_statuses(visited_statuses)
